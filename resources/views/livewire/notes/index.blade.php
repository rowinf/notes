<div
    ...
    @if ($this->form->note->id)
        <div class="lg:hidden px-6 pt-3">
            <div class="flex">
                <flux:button href="{{ route('note.index') }}" class="px-0! gap-0!" size="sm" icon:variant="micro" variant="ghost"
                    icon:leading="icon-chevron-right" wire:navigate>Go Back</flux:button>
                <flux:spacer />

                <flux:button variant="ghost" icon="icon-delete" icon:variant="micro" size="sm" x-on:click="deleteDialogOpen = true; $nextTick(() => $refs.deleteDialog.showModal())">
                </flux:button>
                ...
                @else
                    <flux:button variant="ghost" icon="icon-archive" icon:variant="micro" size="sm" x-on:click="archiveDialogOpen = true; $nextTick(() => $refs.archiveDialog.showModal())">
                    </flux:button>
                @endif
    ...
    @if ($this->form->note->id)
        <div class="flex-0 border-l py-5 px-4 hidden lg:block">
