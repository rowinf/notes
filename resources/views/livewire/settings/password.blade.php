...
<x-settings.layout heading="{{ __('Change password') }}">
    <form wire:submit="updatePassword" class="flex flex-col mt-6 space-y-6 max-w-[528px]">
        <flux:input wire:model="current_password" id="update_password_current_passwordpassword"
            label="{{ __('Old password') }}" type="password" name="current_password" required
            autocomplete="current-password" />
        <flux:field>
            <flux:input wire:model="password" id="update_password_password" label="{{ __('New password') }}"
                type="password" name="password" required autocomplete="new-password" />
            <flux:description class="mt-0! flex gap-1 items-center"><flux:icon.icon-info class="size-5" /> At least 8
                characters</flux:description>
        </flux:field>

        <flux:input wire:model="password_confirmation" id="update_password_password_confirmation"
            label="{{ __('Confirm New Password') }}" type="password" name="password_confirmation" required
